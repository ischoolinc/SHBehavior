...
namespace 德行成績試算表
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            ClassReports["報表"]["學務相關報表"]["日常表現記錄表(Docx)"].Click += delegate
            {
                NewDocxForm calc = new NewDocxForm();
                calc.ShowDialog();
            };
            ...
            K12.Presentation.NLDPanels.Class.SelectedSourceChanged += delegate
            {
                ...
                ClassReports["報表"]["學務相關報表"]["日常表現記錄表(Docx)"].Enable = (K12.Presentation.NLDPanels.Class.SelectedSource.Count > 0) && Permissions.日常表現記錄表_docx權限;
            };

            Catalog ribbon = RoleAclSource.Instance["班級"]["報表"];
            ribbon.Add(new RibbonFeature(Permissions.日常表現記錄表, "日常表現記錄表(新制)"));
            ribbon.Add(new RibbonFeature(Permissions.日常表現記錄表_docx, "日常表現記錄表(Docx)"));
        }
    }
}
