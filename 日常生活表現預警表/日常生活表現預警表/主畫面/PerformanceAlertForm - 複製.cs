...
namespace 日常生活表現預警表
{
    public partial class PerformanceAlertForm_Copy : BaseForm
    {
        ...
        public PerformanceAlertForm_Copy()
        {
            InitializeComponent();
        }
        ...
        private void PerformanceAlertForm_Load(object sender, EventArgs e)
        {
            ...
            bgw_ConfigSave.RunWorkerCompleted += Bgw_ConfigSave_RunWorkerCompleted;
            bgw_ConfigSave.DoWork += Bgw_ConfigSave_DoWork;
...
order by class.grade_year,class.display_order,class.class_name,student.seat_no,student.name", string.Join(",", classIDList))) ;
        ...
        private void Bgw_ConfigSave_DoWork(object sender, DoWorkEventArgs e)
        {
            ConfigData cd = K12.Data.School.Configuration[ConfigName2];
        ...
        private void Bgw_ConfigSave_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            //如果要再存一次
            if (SaveStatus)
            {
                bgw_ConfigSave.RunWorkerAsync();
            }
        }
        ...
        private void cbIsMessage_CheckedChanged(object sender, EventArgs e)
        {
            SavePreference();
        }
